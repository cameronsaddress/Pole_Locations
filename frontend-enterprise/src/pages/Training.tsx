...
export default function Training() {
    ...
    const [telemetry, setTelemetry] = useState({ gpu: 0, vram: 0, power: 0 })
...
useEffect(() => {
    ...
    ws.onopen = () => {
        setLogs(prev => [...prev, "[SYS] Connected to Training Cluster Control Plane"])
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-lg px-4 py-1 border-cyan-500 text-cyan-500 animate-pulse">
                    ...
                    {stats ? `Training Active: Epoch ${stats.epoch}/${stats.total_epochs || 20}` : (trials.length > 0 ? "Tuning Active" : "System Idle")}
                </Badge>
            </div>

            <Tabs defaultValue="detector" className="w-full" onValueChange={setActiveTab}>
                <TabsList className="grid w-full grid-cols-2 bg-black/40 border border-primary/20">
                    <TabsTrigger value="detector" className="data-[state=active]:bg-cyan-950 data-[state=active]:text-cyan-400">
                        <Cpu className="w-4 h-4 mr-2" /> Detector Pipeline (YOLOv8)
                    </TabsTrigger>
                    <TabsTrigger value="clip" className="data-[state=active]:bg-purple-950 data-[state=active]:text-purple-400">
                        <Eye className="w-4 h-4 mr-2" /> Classifier Pipeline (CLIP Zero-Shot)
                    </TabsTrigger>
                </TabsList>
