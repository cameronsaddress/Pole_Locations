...
def train_yolo(lr0, momentum, weight_decay, epochs, batch_size):
    ...
    def on_train_batch_end(trainer):
        # UI Visibility: Log progress FREQUENTLY
        try:
            total = trainer.num_batches
            current = trainer.batch + 1
            # Log every batch for the first 10, then every 5%
            should_log = (current <= 10) or (current % max(1, int(total * 0.05)) == 0) or (current == total)
